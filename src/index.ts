...
function padStart(string: string | number, length: number, pad: string): string | number {
 ...
 * 日 - 中文的日期
 */
const REGEX_FORMAT = /\[([^\]]+)\]|Y{1,4}|M{1,4}|D{1,2}|[年月日]/g
...
const lunar: PluginFunc<string> = (_option, dayjsClass) => {
  dayjsClass.prototype.lunar = function (template) {
    ...
    const matches = (match: string): string | number | null => {
      switch (match) {
        case 'YY':
